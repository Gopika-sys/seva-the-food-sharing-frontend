...
const HomePage = ({ data }) => {
  // Safe fallback if data is undefined or empty
    ...
    <>
      ...
      <div className={styles.main}>
        ...
        <div className={styles.food_required_section}>
          ...
          {safeData.length >= 3 &&
            [0, 1, 2].map((i) => (
              <Link key={safeData[i].id} to={`all/${i}`}>
                <NGOCard data={safeData[i]} />
              </Link>
            ))}
        </div>
        ...
        <div className={styles.food_required_section}>
          ...
          <div className={styles.nearby_images}>
            {safeData.slice(0, 2).map((el, idx) => (
              <Link key={idx} to={`all/${idx}`}>
                <img
